                    ...
                    <div class="bg-white rounded-lg shadow-md p-4">
                        ...
                        @if ($item['type'] === 'lesson')
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <p class="text-gray-800 font-bold">{{ $item['annual_price'] }} ر.س / سنويا</p>
                            </div>
                        @else
                        ...
                        <div class="flex justify-between items-center">
                            @if ($item['type'] === 'lesson')
                                <select class="border rounded-md px-2 py-1 mr-2 plan-selector"
                                    data-item-id="{{ $item['id'] }}" data-type="{{ $item['type'] }}">
                                    <option value="monthly">{{ __('Monthly') }}</option>
                                    <option value="annual">{{ __('Yearly') }}</option>
                                </select>
                            @endif
                            <button
                                class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded remove-from-cart"
                                data-item-id="{{ $item['id'] }}" data-type="{{ $item['type'] }}">
                                {{ __('Remove') }}
                            </button>
                        </div>
            ...
            @else
                <form class="space-y-4" id="login-form">
                    ...
                    <div>
                        ...
                        <input type="password" id="password" name="password" class="border rounded-md px-2 py-1 w-full"
                            required>
                    </div>
