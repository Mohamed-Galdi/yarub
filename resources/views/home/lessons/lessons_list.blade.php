    ...
    <div class="mx-auto max-w-screen-xl py-6 px-2 space-y-12 text-primary pb-12 min-h-screen">
        ...
        <div id="lessonsList" class="grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3  gap-12  px-8 place-items-center">
            @forelse ($lessons as $lesson)
                <x-card.guest-lesson class="w-[22rem] " data-type="{{ $lesson->type }}" :id="$lesson->id" :title="$lesson->title" :description="$lesson->description"
                    :monthly-price="$lesson->monthly_price" :annual-price="$lesson->annual_price" :type="$lesson->type" :averagerating="$lesson->reviews_avg_rating" :totalreviews="$lesson->reviews_count" />
            @empty
